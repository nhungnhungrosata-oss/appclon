...
test('admin login exposes Vbee provider state without secrets',async()=>{
  ...
  const d=await (await req('session')).json(); assert.equal(d.username,'admin'); assert.equal(d.role,'admin'); assert.ok('vbee' in d.providers); assert.ok(!('fish' in d.providers)); assert.ok(!('lipSync' in d));
