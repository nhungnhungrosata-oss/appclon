...
const pages={media:'Tư liệu video',script:'Viết kịch bản',voice:'Giọng Vbee',settings:'Thiết lập'};
let toastTimer;
...
function loginScreen(){
  ...
  $('#login-screen').innerHTML=`<div class="login-wrap"><div class="login-intro"><div class="brand">${logo}<div><strong>cliplab<span class="hero-accent">.</span></strong><small>AI CREATOR STUDIO</small></div></div><span class="eyebrow">VBEE MULTI-USER</span><h1>Tạo nội dung và<br><span class="hero-accent">giọng nói được phân quyền.</span></h1><p>Admin quản lý giọng Vbee cho từng tài khoản; tài khoản con chỉ dùng giọng được cấp.</p><div class="row"><span class="pill green">${icon('shield')} API key ở máy chủ</span><span class="pill purple">Vbee AIVoice</span></div></div><form class="login-card" id="login-form"><h2>Đăng nhập</h2><div class="field"><label>Tên đăng nhập</label><input id="username" autocomplete="username" required maxlength="50"></div><div class="field"><label>Mật khẩu</label><input id="password" type="password" autocomplete="current-password" required maxlength="500"></div><button id="login-btn" class="primary full">Vào studio</button><p id="login-error" class="login-error"></p></form></div>`;
...
function settingsMarkup(){
 ...
 <section class="panel"><div class="panel-head"><h2>An toàn</h2>${icon('shield')}</div><table class="settings-table"><tbody><tr><td>Vbee</td><td>Chỉ dùng giọng admin đã cấp.</td></tr><tr><td>Tài khoản con</td><td>Không thấy API key và không tự đổi voice code.</td></tr><tr><td>Redis</td><td>Cần cho phân quyền nhiều tài khoản trên Vercel.</td></tr></tbody></table><div class="divider"></div><button id="clear-local" class="danger small">${icon('trash')} Xóa media cục bộ</button></section></div>
 ${admin?`<section class="panel admin-panel"><div class="panel-head"><h2>Admin · quản lý giọng và tài khoản</h2><span class="pill purple">ADMIN ONLY</span></div><div class="grid2"><div><h3>Danh sách giọng Nhân bản chuyên nghiệp</h3><p class="tiny muted status-line">Copy mã giọng từ mục Giọng của tôi trên Vbee. Chỉ thêm các giọng Nhân bản chuyên nghiệp mà tài khoản Vbee của anh có quyền sử dụng.</p><div class="field"><label>Tên hiển thị</label><input id="admin-voice-label" maxlength="80" placeholder="Giọng Nhung Pro"></div><div class="field"><label>Voice code Vbee</label><input id="admin-voice-code" maxlength="180" placeholder="Mã giọng đã copy từ Vbee"></div><button id="admin-add-voice" class="primary">Thêm giọng</button><div class="divider"></div><div id="admin-voices" class="stack"></div></div><div><h3>Gán giọng theo tài khoản</h3><p class="tiny muted status-line">Mỗi tài khoản chỉ thấy và dùng giọng được chọn ở đây.</p><div id="admin-users" class="stack"></div></div></div></section>`:''}`;
}
function renderApp(){
...
function showAudio(rec){S.latestAudio=rec;$('#audio-output').hidden=false;$('#tts-preview').src=blobUrl(rec);$('#audio-duration').textContent=clock(rec.duration)}
function renderSettings(){
 const cfg=S.session;
 const defs=[['vbee','Vbee AIVoice','VBEE_APP_ID + VBEE_ACCESS_TOKEN'],['google','Google Gemini','GOOGLE_API_KEY'],['deepseek','DeepSeek','DEEPSEEK_API_KEY'],['openai','OpenAI','OPENAI_API_KEY']];
...
function bindEvents(){
 ...
 $('#app').onchange=e=>{const el=e.target.closest('[data-assign-user]');if(el)busy(el,async()=>{await api('admin-assign-voice',{username:el.dataset.assignUser,voiceCode:el.value});await loadAdminState();if(el.dataset.assignUser===S.session.username)S.session=await api('session');toast('Đã cập nhật giọng cho '+el.dataset.assignUser)})};
 ...
 $('#generate-voice').onclick=()=>busy($('#generate-voice'),async()=>{ensureProvider('vbee');requireConsent('#tts-consent');const sub=await api('tts-submit',{text:$('#voice-text').value,speed:Number($('#voice-speed').value),consent:true});let ready=false;for(let i=0;i<50;i++){await pause(1500);const s=await api('tts-status',{token:sub.token});if(s.failed)throw new Error('Vbee tạo audio thất bại.');if(s.ready){ready=true;break}$('#voice-status').textContent=`Vbee đang xử lý... ${i+1}/50`}if(!ready)throw new Error('Vbee chưa xử lý xong trong thời gian chờ.');const blob=await api('tts-download',{token:sub.token});const duration=await durationOf(blob,'audio');const rec={id:crypto.randomUUID(),name:`vbee-${Date.now()}.mp3`,kind:'audio',blob,duration,source:'vbee',createdAt:Date.now()};await put('assets',rec);S.assets=await all('assets');showAudio(rec);$('#voice-status').textContent=`Đã tạo bằng ${sub.voice.label} · ${clock(duration)}`},'#voice-status');
 $('#download-audio').onclick=()=>{if(S.latestAudio)download(S.latestAudio.blob,S.latestAudio.name)};
 ...
 if($('#admin-add-voice'))$('#admin-add-voice').onclick=()=>busy($('#admin-add-voice'),async()=>{await api('admin-add-voice',{label:$('#admin-voice-label').value,code:$('#admin-voice-code').value});$('#admin-voice-label').value='';$('#admin-voice-code').value='';await loadAdminState();toast('Đã thêm giọng Vbee chuyên nghiệp.')});
