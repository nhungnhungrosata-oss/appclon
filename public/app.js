...
function showAudio(rec){S.latestAudio=rec;$('#audio-output').hidden=false;$('#tts-preview').src=rec.remoteUrl||blobUrl(rec);$('#audio-duration').textContent=rec.remoteUrl?'Link Vbee tạm thời':clock(rec.duration)}
function renderSettings(){
 const cfg=S.session;
 const defs=[['vbee','Vbee AIVoice','VBEE_APP_ID + VBEE_TOKEN'],['google','Google Gemini','GOOGLE_API_KEY'],['deepseek','DeepSeek','DEEPSEEK_API_KEY'],['openai','OpenAI','OPENAI_API_KEY']];
...
function bindEvents(){
 ...
 $('#generate-voice').onclick=()=>busy($('#generate-voice'),async()=>{ensureProvider('vbee');requireConsent('#tts-consent');$('#voice-status').textContent='Đang gửi nội dung sang Vbee...';const sub=await api('tts-submit',{text:$('#voice-text').value,speed:Number($('#voice-speed').value),consent:true});let state=null;for(let i=0;i<120;i++){await pause(i<8?1500:2500);state=await api('tts-status',{token:sub.token});if(state.failed)throw new Error(state.error||'Vbee tạo audio thất bại.');if(state.ready)break;$('#voice-status').textContent=`Vbee đang xử lý... ${i+1}/120`}if(!state?.ready||!state.audioLink)throw new Error('Vbee xử lý lâu hơn dự kiến. Hãy thử lại sau ít phút.');const rec={id:crypto.randomUUID(),name:`vbee-${Date.now()}.mp3`,kind:'audio',remoteUrl:state.audioLink,source:'vbee',createdAt:Date.now()};showAudio(rec);$('#voice-status').textContent=`Đã tạo bằng ${sub.voice.label}. Link tải Vbee có thời hạn ngắn, hãy tải ngay.`},'#voice-status');
