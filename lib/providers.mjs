...
function vbeeToken() {
  ...
  if (!token) fail(503, 'Thiếu VBEE_TOKEN trên Vercel. Có thể tạm dùng VBEE_ACCESS_TOKEN cũ.', 'MISSING_KEY');
  return token;
}
...
async function vbeeJson(url, options = {}, timeout = 20000) {
  ...
  try {
    ...
    if (e?.name === 'TimeoutError' || e?.name === 'AbortError') fail(504, 'Vbee phản hồi quá chậm. Hãy thử lại.', 'VBEE_TIMEOUT');
    fail(502, 'Không kết nối được api.vbee.vn.', 'VBEE_NETWORK_ERROR');
  }
  const raw = await response.text();
  let data = {};
  if (raw) {
    try { data = JSON.parse(raw); }
    catch { fail(502, `Vbee trả dữ liệu không hợp lệ (HTTP ${response.status}).`, 'VBEE_BAD_RESPONSE'); }
  }
  if (!response.ok) {
    const message = data?.error?.message || data?.message || data?.error_message || data?.error || response.statusText || 'Vbee từ chối yêu cầu';
    fail(response.status === 429 ? 429 : 502, `Vbee: ${String(message).slice(0, 300)} (HTTP ${response.status}).`, 'VBEE_API_ERROR');
  }
  return data;
}
...
export async function vbeeSubmit(user, b, callbackBase) {
  ...
  if (!requestId || !/^[a-zA-Z0-9-]{8,100}$/.test(String(requestId))) {
    ...
    fail(502, providerMessage ? `Vbee: ${String(providerMessage).slice(0, 300)}` : 'Vbee không trả requestId hợp lệ.', 'VBEE_BAD_RESPONSE');
  }
...
export async function vbeeDownload(user, token) {
  ...
  if (state.failed) fail(502, state.error || 'Vbee xử lý audio thất bại.', 'VBEE_PROCESSING_FAILED');
  if (!state.ready || !state.audioLink) fail(409, 'Audio Vbee chưa sẵn sàng.', 'VBEE_NOT_READY');
  let u;
  try { u = new URL(state.audioLink); }
  catch { fail(502, 'Vbee trả audioLink không hợp lệ.', 'VBEE_BAD_RESPONSE'); }
  if (u.protocol !== 'https:') fail(502, 'Vbee trả audioLink không an toàn.', 'VBEE_BAD_RESPONSE');
