...
export async function vbeeSubmit(user, b, callbackBase) {
  ...
  const r = await upstream(`${vbeeBase()}/tts`, {
    method: 'POST',
...
export async function vbeeStatus(user, token) {
  ...
  const r = await upstream(`${vbeeBase()}/tts/${encodeURIComponent(t.requestId)}`, {
    headers: { Authorization: `Bearer ${key('VBEE_ACCESS_TOKEN')}` }
  }, 'Vbee', 15000);
  const d = await r.json();
