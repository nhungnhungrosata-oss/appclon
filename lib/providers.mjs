...
async function googleGenerateAnalysis(parts, preferredModel) {
  ...
  for (const model of candidates) {
    ...
    try {
      ...
      if (e?.name === 'TimeoutError' || e?.name === 'AbortError') fail(504, 'Google Gemini phản hồi quá chậm.', 'GOOGLE_TIMEOUT');
    ...
    if (!response.ok) {
      ...
      const reason = response.status === 401 || response.status === 403 ? 'API key hoặc quyền truy cập không hợp lệ'
        ...
        : response.status === 429 ? 'đạt giới hạn, hãy giảm tần suất'
        : 'từ chối dữ liệu hoặc đang lỗi';
      fail(response.status === 429 ? 429 : 502, `Google Gemini: ${reason} (HTTP ${response.status}).`, 'UPSTREAM_ERROR');
    }
    let data;
